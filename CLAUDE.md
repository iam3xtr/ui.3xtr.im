...
Follow the recent repository convention: write the subject in English using a
Conventional Commit type (`feat:`, `fix:`, `refactor:`, `docs:`), in imperative
lowercase, and keep it concise. For a non-trivial change, add a blank line and
an English bullet list beginning with `- `; each item describes one delivered
outcome, not an implementation play-by-play. Mention tests or verification only
when it is a material part of the change. Do not include issue-planning chatter,
assistant attribution, or an autogenerated footer.
