...
- `src/components/UiKit.vue` — витрина-справочник контрактов Stage A3 (кнопки, таблицы, вкладки и т.д.), страница `/kit`; включает эталонную матрицу всех шести значений `DEMO_MODES` (`ready`/`loading`/`empty`/`error`/`permission-denied`/`partial`) для контрактов `ListAsyncState`/`AsyncState`/`partial`-баннера, которыми реально пользуются маршруты (Task A7.6).
