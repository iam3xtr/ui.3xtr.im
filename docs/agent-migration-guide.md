...
- Любой список записей с одинаковым набором полей (пользователи, счета,
  логи, тарифы, API-ключи и т.п.) заворачивается в `b-table`, а не рисуется
  вручную через `<table>` или `div`-сетку.
...
- Таблица оборачивается в `.tr-card`, заголовок — `.tr-card__title` внутри
  ...
  таблицы.
- Единственное исключение — построчный список "файл/медиа + иконка +
  метаданные + действие", где строка визуально сложнее одной строки
  таблицы (пример: список элементов коллекции в `Knowledge.vue`,
  `.tr-knowledge__item`). Такой паттерн не дублировать под новым именем —
  если он понадобится ещё раз, переиспользовать именно
  `tr-knowledge__item`-разметку, а не заводить третий вариант.
