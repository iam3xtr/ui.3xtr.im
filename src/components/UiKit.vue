<template>
  <!--
    Справочник контрактов дизайн-системы (Task A5.3): не подборка Buefy-
    демо ради демо, а живой каталог компонентов/паттернов/состояний, на
    которые ссылаются docs/design-system.md и docs/agent-migration-guide.md
    как на эталон при портировании в get.3xtr.im.
  -->
          <PageHeader
          ...
          <section class="tr-card mb-5">
            ...
            <p class="tr-muted mb-4">
              ...
              и переход на <code>/kit</code> не меняет persisted demo-режим.
            </p>
...
// маршруты Task A7.3–A7.5), а не его текущий режим/флаги. Открытие `/kit`
// поэтому не читает и не переписывает persisted `mode`: карточки ниже сами
// перечисляют шесть значений `DEMO_MODES`, глобальный режим виден отдельной
// строкой (`currentDemoModeLabel`) только для справки.
